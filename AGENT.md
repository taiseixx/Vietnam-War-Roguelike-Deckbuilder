...
| 11 | **Hệ thống Buff động & `baseAtk`**: Tránh thiết lập static-override hoặc ghi đè trực diện thuộc tính `atk` bằng các auras hiện diện (presence auras). Luôn duy trì `baseAtk` là lực tấn công cơ bản (bao gồm các buff vĩnh viễn từ spell), thực hiện tính toán aura động trong `applyAuraBuffs` lồng từ `baseAtk`. Tuyệt đối không cho phép HQ nhận buff tấn công (luôn giữ chặn bằng 0). |
